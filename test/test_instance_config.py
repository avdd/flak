...
from flak._compat import PY2


def test_explicit_instance_paths(modules_tmpdir):
    app = Flak(__name__, instance_path='instance')
    assert app.instance_path.endswith('instance')

    app = Flak(__name__, instance_path=str(modules_tmpdir))
    assert app.instance_path == str(modules_tmpdir)


def test_main_module_paths(modules_tmpdir, purge_module):
    app = modules_tmpdir.join('main_app.py')
    app.write('from flak import Flak\n\napp = Flak("__main__")')
...
def test_installed_package_paths(limit_loader, modules_tmpdir,
    ...
    init = app.join('__init__.py')
    init.write('from flak import Flak\napp = Flak(__name__)')
